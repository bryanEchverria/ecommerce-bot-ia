...
async def create_order(order: OrderCreate, order_id: str = None, db: AsyncSession = Depends(get_async_db)):
    """Create a new order with auto-generated order number"""
    if order_id is None:
        order_id = str(uuid4())
    return await crud_async.create_order_async(db=db, order=order.dict(), order_id=order_id)

@router.put("/orders/{order_id}", response_model=Order)
async def update_order(order_id: str, order: OrderUpdate, db: AsyncSession = Depends(get_async_db)):
