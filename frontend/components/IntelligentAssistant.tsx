...
`;
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-28 right-8 w-full max-w-md bg-surface rounded-xl shadow-2xl z-50 flex flex-col animate-fade-in-up">
          <header className="p-4 border-b border-white/10 flex justify-between items-center">
            <h3 className="text-lg font-semibold text-on-surface">{t('assistant.title')}</h3>
            <button onClick={() => setIsOpen(false)} className="text-on-surface-secondary hover:text-on-surface p-1 rounded-full hover:bg-white/10">
          ...
          <div className="flex-1 p-4 space-y-4 overflow-y-auto h-96">
            {messages.length === 0 && !isLoading && (
              <div className="text-center p-4">
                 ...
                 <p className="text-on-surface font-semibold">{t('assistant.welcomeMessage')}</p>
                 <div className="mt-4 space-y-2 text-sm">
                    <button onClick={() => handleSendMessage(t('assistant.initialPrompt1'))} className="w-full text-left bg-secondary p-2 rounded-lg hover:bg-white/10 transition-colors text-on-surface-secondary" disabled={!ai}>{t('assistant.initialPrompt1')}</button>
                    <button onClick={() => handleSendMessage(t('assistant.initialPrompt2'))} className="w-full text-left bg-secondary p-2 rounded-lg hover:bg-white/10 transition-colors text-on-surface-secondary" disabled={!ai}>{t('assistant.initialPrompt2')}</button>
                    <button onClick={() => handleSendMessage(t('assistant.initialPrompt3'))} className="w-full text-left bg-secondary p-2 rounded-lg hover:bg-white/10 transition-colors text-on-surface-secondary" disabled={!ai}>{t('assistant.initialPrompt3')}</button>
                 </div>
              </div>
            )}
            {messages.map((msg) => (
              <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.sender === 'ai' && (
                   <div className="flex-shrink-0 bg-secondary p-2 rounded-full">
                     <BotIcon className="w-5 h-5 text-on-surface-secondary" />
                   </div>
                )}
                <div className={`max-w-xs md:max-w-sm rounded-2xl px-4 py-2 ${msg.sender === 'user' ? 'bg-primary text-white rounded-br-none' : 'bg-secondary text-on-surface rounded-bl-none'}`}>
             ...
             {isLoading && (
                <div className="flex items-end gap-2 justify-start">
                    ...
                    <div className="max-w-xs rounded-2xl px-4 py-2 bg-secondary text-on-surface rounded-bl-none">
                        <TypingIndicator />
                    </div>
                </div>
            )}
          ...
          <form onSubmit={handleFormSubmit} className="p-4 border-t border-white/10">
            <div className="relative">
              <input
                ...
                className="w-full bg-background border border-white/10 rounded-lg py-2 pl-3 pr-10 focus:outline-none focus:ring-2 focus:ring-primary disabled:opacity-50"
                disabled={isLoading || !ai}
              />
              <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-on-surface-secondary hover:text-primary disabled:text-gray-600" disabled={isLoading || !input.trim() || !ai}>
