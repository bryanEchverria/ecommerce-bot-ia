...
const ClientDetail: React.FC<ClientDetailProps> = ({ client, onBack }) => {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
             <button
                onClick={onBack}
                className="flex items-center gap-2 text-on-surface-secondary hover:text-on-surface font-semibold py-2 px-3 rounded-lg hover:bg-surface transition-colors mb-4 -ml-3"
            >
            ...
            {/* Client Info Card */}
            <div className="bg-surface rounded-xl shadow-lg p-6 flex flex-col md:flex-row items-start gap-6">
                <img src={client.avatarUrl} alt={client.name} className="h-24 w-24 rounded-full border-4 border-primary" />
                <div className="flex-grow">
                    <h2 className="text-2xl font-bold text-on-surface">{client.name}</h2>
                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-on-surface-secondary">
                ...
                <div className="w-full md:w-auto text-center md:text-right border-t md:border-t-0 md:border-l border-white/10 pt-4 md:pt-0 md:pl-6 mt-4 md:mt-0">
                    <p className="text-sm text-on-surface-secondary font-medium">{t('clients.totalSpentLabel')}</p>
            ...
            <div className="bg-surface rounded-xl shadow-lg">
                <div className="p-6 border-b border-white/10">
                    <h3 className="text-xl font-semibold text-on-surface">{t('clients.orderHistory')}</h3>
                    <p className="text-sm text-on-surface-secondary mt-1">{t('clients.orderHistoryDesc', { name: client.name })}</p>
                </div>
